...
/* required globals for X and visuals */
static Display *dpy;
static int screen;
static Visual *visual;
static int depth;
static XVisualInfo template;
void
xinitvisual(void) {
    XVisualInfo *infos;
    ...
    for (infos = XGetVisualInfo(dpy, VisualScreenMask, &template, &nitems); nitems; infos++, nitems--) {
        fmt = XRenderFindVisualFormat(dpy, infos->visual);
		...
		if (text[i] == '^' && !isCode) {
			...
			while (text[++i] != '^') {
				if (text[i] == 'c') {
					...
					buf[7] = '\0';
					drw_clr_create(drw, &drw->scheme[ColFg], buf, 0xdd);
					i += 7;
				} else if (text[i] == 'b') {
					char buf[8];
					memcpy(buf, (char*)text+i+1, 7);
...
setup(void)
{
	...
	Atom utf8string;
	struct sigaction sa;
	static const unsigned int alphas[] = {0xdd, 0xbb, 0xff}; // fg, bg, border
